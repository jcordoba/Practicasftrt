...
export default function UnionesPage() {
  ...
  const fetchUnions = async () => {
    try {
      ...
      if (!response.ok) {
        const message = await getResponseMessage(response, 'Error al cargar uniones');
        throw new Error(message);
      }

      const data = await response.json();
      setUnions((data || []).map((union: any) => ({
        id: union.id,
        name: union.name ?? union.nombre ?? '',
        description: union.description ?? union.descripcion,
        createdAt: union.createdAt ?? union.fecha_creacion,
        updatedAt: union.updatedAt ?? union.fecha_actualizacion,
      })));
    } catch (error) {
  ...
  const handleUpdateUnion = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authenticatedFetch(`/api/unions/${editingUnion.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          nombre: editingUnion.name,
          descripcion: editingUnion.description || undefined
        })
      });
      ...
      <main className="flex flex-col items-center w-full max-w-6xl mt-8 px-4">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 w-full overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 bg-slate-50">
            <h3 className="text-lg font-semibold !text-slate-900">Uniones Registradas</h3>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {unions.map((union) => (
                  <tr key={union.id} className="hover:bg-blue-50/40 transition-colors">
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-lg rounded-2xl border border-slate-200 shadow-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                <h3 className="text-xl font-semibold !text-slate-900">Editar Unión</h3>
                <button
                  type="button"
              ...
              <form onSubmit={handleUpdateUnion} className="p-6 space-y-5">
                <div>
