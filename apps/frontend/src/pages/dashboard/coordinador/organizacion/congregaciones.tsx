...
interface CreateCongregationForm {
  name: string;
  description: string;
  districtId: string;
}
...
export default function CongregacionesPage() {
  ...
  const [createForm, setCreateForm] = useState<CreateCongregationForm>({
    name: '',
    description: '',
    districtId: ''
  });
  ...
  const fetchData = async () => {
    try {
      const [congregationsRes, districtsRes] = await Promise.all([
        authenticatedFetch('/api/congregations'),
  ...
  const handleUpdateCongregation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Error al actualizar congregación');
      }
      ...
      setError('Error al actualizar la congregación');
    }
  };
  ...
  const handleDeleteCongregation = async (id: string) => {
    ...
    try {
      const response = await authenticatedFetch(`/api/congregations/${id}`, {
        method: 'DELETE'
      });

      if (!response.ok) {
        throw new Error('Error al eliminar congregación');
      }

      fetchData();
    } catch (error) {
      console.error('Error deleting congregation:', error);
      setError('Error al eliminar la congregación');
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-lg">Cargando congregaciones...</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-slate-100">
      ...
      <main className="flex flex-col items-center w-full max-w-6xl mt-8 px-4">
        ...
        {/* Botón Crear Nueva Congregación */}
        ...
        {showCreateForm && (
          <div className="bg-white rounded-lg shadow p-6 w-full mb-6 border-2 border-green-600">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium !text-slate-900">Crear Nueva Congregación</h3>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateCongregation} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                  ...
                  <tr key={congregation.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm !text-slate-700">
                        {congregation.district?.name || 'Sin distrito asignado'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm !text-slate-700">{congregation.description || 'Sin descripción'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm !text-slate-700">
                        {new Date(congregation.createdAt).toLocaleDateString()}
                      </div>
                    </td>
